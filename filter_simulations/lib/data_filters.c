...
// queue_s* optimal_filter_cheby(queue_s* queue, int fs, float lp_cutoff, float hp_cutoff, bool high_pass, bool invert_flag, bool smooth_SG, bool savgol_param = NULL) {
//     // Define output queue
//     queue_s* output_queue = malloc(sizeof(queue_s));
//     initialize(output_queue, queue->full);
...
BWBandPass* create_bw_band_pass_filter(int order, float s, float fl, float fu){
    ...
    BWBandPass* filter = malloc(sizeof(BWBandPass));
    filter -> n = (int)order/4;
